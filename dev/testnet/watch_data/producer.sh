...
#  if [ $index -eq 0 ]; then
      exchaind add-genesis-account $(exchaincli keys show alice -a) 10000okt --home cache/node${index}/exchaind
      exchaind add-genesis-account $(exchaincli keys show bob -a) 10000okt --home cache/node${index}/exchaind
#  fi
  ...
  echorun nohup exchaind start \
    ...
    --rest.unlock_key turing,alice,bob \
    --rest.unlock_key_home "~/.exchaincli" \
    --keyring-backend "test" \
