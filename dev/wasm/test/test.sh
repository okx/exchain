...
echo "0-----------------------"
exchaincli tx send captain $(exchaincli keys show fred -a) 1000okb --fees 0.001okb -y -b block
...
exchaincli tx wasm migrate "$CONTRACT" "$BURNER_CODE_ID" "{\"payout\": \"$DEST_ACCOUNT\"}" --from fred  --fees 0.001okb \
 -b block -y -o json | jq
...
echo "### Query old admin: $(exchaincli q wasm contract "$CONTRACT" -o json | jq -r '.contract_info.admin')"
echo "### Update contract"
exchaincli tx wasm clear-contract-admin "$CONTRACT" --fees 0.001okb \
