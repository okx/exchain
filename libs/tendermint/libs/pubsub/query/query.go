...
func ExpandEvents(flattenedEvents map[string][]string) []types.EventAdapter {
	events := make([]types.EventAdapter, 0)

	for composite, values := range flattenedEvents {
		tokens := strings.Split(composite, ".")

		attrs := make([]types.EventAttributeAdapter, len(values))
		for i, v := range values {
			attrs[i] = types.EventAttributeAdapter{
				Key:   tokens[len(tokens)-1],
				Value: v,
			}
		}

		events = append(events, types.EventAdapter{
...
func (q *Query) matchesEvents(events []types.EventAdapter) bool {
...
// condition tag.
func (c condition) findAttr(event types.EventAdapter) ([]string, bool) {
...
func (c condition) matchesAny(events []types.EventAdapter) bool {
...
func (c condition) matchesEvent(event types.EventAdapter) bool {
