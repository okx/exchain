...
func DeltaContextWithListener(l listener.IListenerComponent) DeltaContextOption {
	return func(ctx *DeltaContext) {
		ctx.listener = l
	}
}
func DeltaContextWithBroker(broker delta.DeltaBroker) DeltaContextOption {
	return func(ctx *DeltaContext) {
		ctx.deltaBroker = broker
	}
}

func DeltaContextWithDownloadControl(download bool) DeltaContextOption {
	return func(ctx *DeltaContext) {
		ctx.downloadDelta = download
	}
}
func DeltaContextWithUploadControl(upload bool) DeltaContextOption {
	return func(ctx *DeltaContext) {
		ctx.uploadDelta = upload
	}
}
...
type DeltaContextProvider func(l log.Logger) *DeltaContext
...
func newDeltaContext(l log.Logger, ops ...DeltaContextOption) *DeltaContext {
	...
	if dp.downloadDelta && nil != dp.listener {
		dp.dataMap.hook = func(p *types.Deltas) {
			dp.listener.NotifyListener(p, strconv.FormatInt(p.Height, 10))
		}
	} else {
		dp.dataMap.hook = func(_ *types.Deltas) {}
	}
