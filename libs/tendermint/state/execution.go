...
func (blockExec *BlockExecutor) ApplyBlock(
	...
	blockExec.logger.Error("-- After updateState, vals for height:", "height", block.Height)
	blockExec.logger.Error(fmt.Sprintf("%v", state.LastValidators))
	blockExec.logger.Error("-- vals for height:", "height", block.Height+1)
	blockExec.logger.Error(fmt.Sprintf("%v", state.Validators))
	blockExec.logger.Error("-- vals for height:", "height", block.Height+1+1)
	blockExec.logger.Error(fmt.Sprintf("%v", state.NextValidators))
	if err != nil {
