...
import (
	...
	p2pmock "github.com/okex/exchain/libs/tendermint/p2p/mock"
...
func TestCase2PrerunWithCacheButCASFailed(t *testing.T) {
	...
	vertifyPrerunWithDelta(t, func(trace, status int32) {
		...
		if trace == sm.CASE_SPECIAL_DELTA_BEFORE_BEGIN {
			...
			// push again
			//pushQ(t,q)
		} else if trace == sm.CASE_SPECIAL_BEFORE_LOAD_CACHE {
			close(c1) // notify delta ,we have the task
...
func TestCase3D2D1PrerunGetBeginBlockAndRaceConditionPrerunGetRaceEnd(t *testing.T) {
	localDebug(t)
...
func TestCase3D2D2PreRunDontLockBeginBlcokAndReturned(t *testing.T) {
	localDebug(t)
...
func vertifyPrerunWithDelta(t *testing.T, f func(trace, status int32),afterRoundF func(),
	...
	sm.SetTraceHook(func(trace, status int32) {
		...
		go func() {
			...
			defer mtx.Unlock()
			defer wg.Done()
			traceNodes = append(traceNodes, &statusTraceNode{trace: trace, status: status})
		}()
	})
	...
	raceBeginBlockSuccCount := 0
	resultHit := 0
	beginBlockStatusHit := 0
	raceEndFailCount := 0
	raceEndSuccCount := 0
	for _, node := range traceNodes {
		if node.trace&sm.RACE_BEGINBLOCK_SUCCESS >= sm.RACE_BEGINBLOCK_SUCCESS {
			...
			raceBeginBlockSuccCount++
		}
		if node.trace&sm.RACE_BEGINBLOCK_FAIL >= sm.RACE_BEGINBLOCK_FAIL {
			raceBeginBlockFailCount++
		}
		if node.trace&sm.RESULT_TRAEC >= sm.RESULT_TRAEC {
			if statusCheckRaceEnd > 0 && node.status&statusCheckRaceEnd >= statusCheckRaceEnd {
				resultHit++
			}
		}
		if node.trace&sm.RACE_END_FAIL >= sm.RACE_END_FAIL {
			raceEndFailCount++
		}
		if node.trace&sm.RACE_END_SUCCESS >= sm.RACE_END_SUCCESS {
			raceEndSuccCount++
		}
	...
	exceptF := func(ex, ac int, name string) {
		if ex < 0 {
			return
		}
		require.Equal(t, ex, ac, name)
	}
	exceptF(exRaceBeginBlockFailCount, raceBeginBlockFailCount, "raceBeginBlockFailCount")
	exceptF(exRaceBeginBlockSuccCount, raceBeginBlockSuccCount, "raceBeginBlockSuccCount")
	exceptF(exRaceEndSuccCount, raceEndSuccCount, "raceEndSuccCount")
	exceptF(exRaceEndFailCount, raceEndFailCount, "raceEndFailCount")

	if statusCheckRaceEnd > 0 {
		exceptF(1, resultHit, "resultHit")
	}
	if statusCheckBeginBlock > 0 {
		exceptF(1, beginBlockStatusHit, "beginBlockStatusHit")
	}
	exceptF(0, disableHit, "disableHit")
