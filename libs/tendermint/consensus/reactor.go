...
func (conR *Reactor) broadcastNewRoundStepMessage(rs *cstypes.RoundState) {
	...
	conR.Switch.Broadcast(StateChannel, cdc.MustMarshalBinaryWithSizer(nrsMsg, false))
}
...
func (conR *Reactor) broadcastHasVoteMessage(vote *types.Vote) {
	...
	conR.Switch.Broadcast(StateChannel, cdc.MustMarshalBinaryWithSizer(msg, false))
	/*
...
func (conR *Reactor) broadcastSignVoteMessage(vote *types.Vote) {
	...
	conR.Switch.Broadcast(VoteChannel, cdc.MustMarshalBinaryWithSizer(msg, false))
}
...
func (conR *Reactor) sendNewRoundStepMessage(peer p2p.Peer) {
	...
	nrsMsg := makeRoundStepMessage(rs)
	peer.Send(StateChannel, cdc.MustMarshalBinaryWithSizer(nrsMsg, false))
}
		...
		// Maybe send Height/Round/Prevotes
		{
			...
			if rs.Height == prs.Height {
				if maj23, ok := rs.Votes.Prevotes(prs.Round).TwoThirdsMajority(); ok {
					peer.TrySend(StateChannel, cdc.MustMarshalBinaryWithSizer(&VoteSetMaj23Message{
						Height:  prs.Height,
						Round:   prs.Round,
...
func (ps *PeerState) PickSendVote(votes types.VoteSetReader) bool {
	if vote, ok := ps.PickVoteToSend(votes); ok {
		...
		if ps.peer.Send(VoteChannel, cdc.MustMarshalBinaryWithSizer(msg, false)) {
			ps.SetHasVote(vote)
			return true
		}
		return false
...
func (m NewRoundStepMessage) AminoSize(_ *amino.Codec) int {
	...
	if m.Step != 0 {
		size += 1 + amino.UvarintSize(uint64(m.Step))
	}
	if m.SecondsSinceStartTime != 0 {
		size += 1 + amino.UvarintSize(uint64(m.SecondsSinceStartTime))
	}
...
func (m NewRoundStepMessage) MarshalAminoTo(_ *amino.Codec, buf *bytes.Buffer) error {
	...
	if m.Step != 0 {
		...
		err = amino.EncodeUvarintWithKeyToBuffer(buf, uint64(m.Step), pbKey)
		if err != nil {
			return err
		}
	}
	...
	if m.SecondsSinceStartTime != 0 {
		...
		err = amino.EncodeUvarintWithKeyToBuffer(buf, uint64(m.SecondsSinceStartTime), pbKey)
		if err != nil {
			return err
		}
	}
...
func (m BlockPartMessage) AminoSize(cdc *amino.Codec) int {
	var size int
	if m.Height != 0 {
		size += 1 + amino.UvarintSize(uint64(m.Height))
	}
...
func (m BlockPartMessage) MarshalAminoTo(cdc *amino.Codec, buf *bytes.Buffer) error {
	var err error
	// field 1
	if m.Height != 0 {
		const pbKey = byte(1<<3 | amino.Typ3_Varint)
	...
	if m.Part != nil {
		...
		buf.WriteByte(pbKey)
		partSize := m.Part.AminoSize(cdc)
		err = amino.EncodeUvarintToBuffer(buf, uint64(partSize))
		if err != nil {
			return err
		}
		lenBeforeData := buf.Len()
		err = m.Part.MarshalAminoTo(cdc, buf)
		if err != nil {
			return err
		}
