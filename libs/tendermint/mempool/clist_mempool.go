	...
	for i, tx := range txs {
		...
		if ele := mem.cleanTx(height, tx, txCode); ele != nil {
			...
			if cfg.DynamicConfig.GetMaxGasUsedPerBlock() > 0 && cfg.DynamicConfig.GetEnablePGU() {
				memTx := ele.Value.(*mempoolTx)
				mem.pguLogger.Info("PGU info", "txHash", hex.EncodeToString(memTx.tx.Hash(mem.Height())), "gasLimit", memTx.gasLimit, "agu", gasUsedPerTx, "egu", memTx.gasWanted, "num", memTx.isSim)
			}
		} else {
...
func (mem *CListMempool) simulateTx(tx types.Tx) (*SimulationResponse, error) {
	...
	var log string
	if simuRes.Result != nil {
		log = simuRes.Result.Log
	}
	fmt.Println("hash", hex.EncodeToString(tx.Hash(mem.Height())), "log:", log, "gasWanted", simuRes.GasWanted, "gasUsed", simuRes.GasUsed)
	return &simuRes, err
}
