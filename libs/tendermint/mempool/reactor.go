...
type mempoolIDs struct {
	...
	activeIDs map[uint16]p2p.ID // used to check if a given peerID key is used, the value doesn't matter
}
...
func (ids *mempoolIDs) GetForPeerID(peer p2p.ID) uint16 {
	ids.mtx.RLock()
	defer ids.mtx.RUnlock()
...
func (ids *mempoolIDs) GetPeerID(id uint16) p2p.ID {
	ids.mtx.RLock()
	pid := ids.activeIDs[id]
	ids.mtx.RUnlock()
	return pid
}
...
func (memR *Reactor) SetLogger(l log.Logger) {
	...
	memR.receiver.SetLogger(l.With("module", "tx-receiver"))
}
...
func (memR *Reactor) getTxJobChannel(from string) chan<- txJob {
	...
	if len(memR.txChs)%2 == 0 {
		...
		if selected == byte(len(memR.txChs)) {
			selected--
			if amino.StrToBytes(from)[len(from)-2]&0x01 == 0 {
				selected--
			}
		}
	}
