...
// source: cosmos/base/v1beta1/CoinAdapter.proto
...
func (m *CoinAdapter) Reset()      { *m = CoinAdapter{} }
...
func (m *CoinAdapter) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	if deterministic {
		return xxx_messageInfo_CoinAdapter.Marshal(b, m, deterministic)
	} else {
		b = b[:cap(b)]
...
func (m *CoinAdapter) XXX_Merge(src proto.Message) {
	xxx_messageInfo_CoinAdapter.Merge(m, src)
}
func (m *CoinAdapter) XXX_Size() int {
	return m.Size()
}
func (m *CoinAdapter) XXX_DiscardUnknown() {
	xxx_messageInfo_CoinAdapter.DiscardUnknown(m)
}

var xxx_messageInfo_CoinAdapter proto.InternalMessageInfo
...
func (this *CoinAdapter) Equal(that interface{}) bool {
	...
	that1, ok := that.(*CoinAdapter)
	if !ok {
		that2, ok := that.(CoinAdapter)
		if ok {
			that1 = &that2
		} else {
			return false
...
func sozCoinAdapter(x uint64) (n int) {
	return sovCoinAdapter(uint64((x << 1) ^ uint64((int64(x) >> 63))))
}
func (m *CoinAdapter) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		if wireType == 4 {
			return fmt.Errorf("proto: CoinAdapter: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: CoinAdapter: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
...
func skipCoinAdapter(dAtA []byte) (n int, err error) {
	...
	for iNdEx < l {
		...
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return 0, ErrIntOverflowCoinAdapter
			}
			if iNdEx >= l {
		...
		case 2:
			...
			if length < 0 {
				return 0, ErrInvalidLengthCoinAdapter
			}
			iNdEx += length
		case 3:
			depth++
		case 4:
			if depth == 0 {
				return 0, ErrUnexpectedEndOfGroupCoinAdapter
			}
			depth--
		case 5:
			iNdEx += 4
