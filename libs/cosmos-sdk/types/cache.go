...
func (c *Cache) Write(handler AccCacheDataHandler, ctx Context, updateDirty bool) {
	if c.Skip() {
		return
	}
	...
	c.writeAcc(handler, ctx, updateDirty)
...
func (c *Cache) writeAcc(handler AccCacheDataHandler, ctx Context, updateDirty bool) {
	for addr, v := range c.accMap {
		...
		if v.isDirty {
			if handler != nil {
				accAddr, _ := AccAddressFromBech32(addr.String())
				handler(ctx, accAddr, v.acc)
			}
		}
	}
