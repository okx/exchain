	...
	if !ctx.IsCheckTx() && !ctx.IsReCheckTx() {
		if ak.observers != nil {
			for _, observer := range ak.observers {
				if observer != nil {
					observer.OnAccountUpdated(acc, false)
				}
			}
		}
	}
...
func (ak AccountKeeper) UpdateAccountInStateDB(ctx sdk.Context, acc exported.Account) {
	if ak.observers != nil {
		for _, observer := range ak.observers {
			if observer != nil {
				observer.OnAccountUpdated(acc, true)
			}
		}
	}
}
