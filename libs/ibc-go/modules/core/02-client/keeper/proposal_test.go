...
//				store := suite.chainA.GetContextPointer().KVStore(suite.chainA.GetSimApp().GetKey(upgradetypes.StoreKey))
//				bz := suite.chainA.App().AppCodec().GetProtocMarshal().MustMarshalBinaryBare(&oldPlan)
