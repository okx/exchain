...
func MigrateGenesis(cdc *codec.CodecProxy, clientGenState *types.GenesisState, genesisBlockTime time.Time, selfHeight exported.Height) (*types.GenesisState, error) {
	...
	for i, client := range clientGenState.Clients {
		...
		if clientType == exported.Solomachine {
			...
			if err := cdc.GetProtocMarshal().UnmarshalBinaryBare(client.ClientState.Value, clientState); err != nil {
