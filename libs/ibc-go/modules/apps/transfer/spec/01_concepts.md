...
It is strongly recommended to read the full details of [ADR 001: Coin Source Tracing](https://github.com/okex/exchain/libs/ibc-go/blob/main/docs/architecture/adr-001-coin-source-tracing.md) to understand the implications and context of the IBC token representations.
...
In some [exceptional cases](https://github.com/okex/exchain/libs/ibc-go/blob/main/docs/architecture/adr-026-ibc-client-recovery-mechanisms.md#exceptional-cases), a client state associated with a given channel cannot be updated. This causes that funds from fungible tokens in that channel will be permanently locked and thus can no longer be transferred.
