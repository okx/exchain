...
func VMBPrecompileSetup(t *testing.T, chain *Chain) {
	...
	deployFreeCallContract(t, chain)
...
func deployFreeCallContract(t *testing.T, chain *Chain) {
	erc20abi, err := abi.JSON(bytes.NewReader(erc20abiBytes))
	require.NoError(t, err)
	chain.erc20ABI = erc20abi
	...
	freeCallBytescode := ethcmn.Hex2Bytes(freeCallPalyload)
	_, r2, err := chain.app.VMBridgeKeeper.CallEvm(chain.Ctx(), ethcmn.BytesToAddress(chain.addr[0]), nil, big.NewInt(0), freeCallBytescode)
	require.NoError(t, err)
	chain.seq[0]++
	chain.freeCallEvmContract = r2.ContractAddress

	freeCallWasmCode, err := ioutil.ReadFile("./testdata/freecall.wasm")
	require.NoError(t, err)
	chain.freeCallWasmCodeId, err = chain.app.WasmPermissionKeeper.Create(chain.Ctx(), sdk.AccToAWasmddress(chain.addr[0]), freeCallWasmCode, nil)
	require.NoError(t, err)
	initFreeCallMsg := []byte(fmt.Sprintf("{\"decimals\":10,\"initial_balances\":[{\"address\":\"%s\",\"amount\":\"100000000\"},{\"address\":\"%s\",\"amount\":\"100000000\"}],\"name\":\"my test token\", \"symbol\":\"MTT\"}", sdk.AccToAWasmddress(chain.addr[0]).String(), sdk.WasmAddress(chain.freeCallEvmContract.Bytes()).String()))
	chain.freeCallWasmContract, _, err = chain.app.WasmPermissionKeeper.Instantiate(chain.Ctx(), chain.freeCallWasmCodeId, sdk.AccToAWasmddress(chain.addr[0]), sdk.AccToAWasmddress(chain.addr[0]), initFreeCallMsg, "label", sdk.Coins{sdk.NewInt64Coin(sdk.DefaultBondDenom, 0)})
	require.NoError(t, err)
}

func createCallToEvmTx(t *testing.T, chain *Chain) []byte {
	caller := chain.freeCallWasmContract.String()
	contract := chain.freeCallEvmContract.String()
	//contractEx := sdk.AccAddress(chain.freeCallEvmContract.Bytes()).String()
	...
	evmInput, err := getCallByWasmInput(chain.erc20ABI, caller, callData)
	require.NoError(t, err)
