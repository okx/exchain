...
func ethGasConsume(ctx *sdk.Context, acc exported.Account, accGetGas sdk.Gas, msgEthTx *evmtypes.MsgEthereumTx, simulate bool, ak auth.AccountKeeper, sk types.SupplyKeeper) error {
	...
	if gasLimit != 0 {
		...
		err = auth.DeductFees(ak, sk, *ctx, acc, feeAmt)
		if err != nil {
			return err
		}
	}
...
func (avd AccountAnteDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (newCtx sdk.Context, err error) {
	...
	if !simulate {
		...
		err = ethGasConsume(&ctx, acc, getAccGasUsed, msgEthTx, simulate, avd.ak, avd.sk)
		acc = nil
