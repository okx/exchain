...
func (pool *TxPool) continueBroadcast(api *PublicEthereumAPI, currentNonce uint64, address common.Address) error {
	...
	if err != nil {
		if !strings.Contains(err.Error(), sdkerrors.ErrMempoolIsFull.Error()) &&
			...
			err = fmt.Errorf("%s, nonce %d of tx [%s] has been dropped, please send again\n",
				err.Error(), pool.addressTxsPool[address][i].Data.AccountNonce, pool.addressTxsPool[address][i].Data.Hash.String())
			pool.dropTxs(i+1, address)
		} else {
			err = fmt.Errorf("%s, nonce %d, hash [%s]\n", err.Error(), pool.addressTxsPool[address][i].Data.AccountNonce, pool.addressTxsPool[address][i].Data.Hash.String())
