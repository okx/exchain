...
func (k *Keeper) FixLog(tx []sdk.Tx, logIndex []int, hasEnterEvmTx []bool, anteErrs []error, msgs [][]sdk.Msg, resp []abci.ResponseDeliverTx) [][]byte {
	txSize := len(logIndex)
	...
	for index := 0; index < txSize; index++ {
		...
		k.saveParallelTxResult(tx[index], msgs[index], rs.ResultData, resp[index])
	}
...
func (k *Keeper) saveParallelTxResult(tx sdk.Tx, msgs []sdk.Msg, resultData *types.ResultData, resp abci.ResponseDeliverTx) {
	if !k.Watcher.Enabled() {
		return
	}
	k.Watcher.SaveParallelTx(tx, msgs, resultData, resp)
}
