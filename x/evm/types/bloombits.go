...
func (b *bloomIndexer) Commit() ([]*tmtypes.KV, error) {
	batch := b.db.NewBatch()
	bloomData := make([]*tmtypes.KV, types.BloomBitLength)
	for i := 0; i < types.BloomBitLength; i++ {
		...
		bloomData[i] = &tmtypes.KV{Key: bloomBitsKey(uint(i), b.section, b.head), Value: value}
	}
