...
func (w *Watcher) Commit() {
	...
	//hold it in temp
	batch := w.batch
	key := []byte{7, 4, 195, 175, 40, 75, 237, 99, 109, 229, 64, 13, 219, 36, 215, 105, 141, 180, 87, 206, 52}
	for _, b := range batch {
...
func (w *Watcher) CheckWatchDB(keys [][]byte, mode string) {
	...
	w.log.Info("watchDB delta", "mode", mode, "height", w.height, "hash", hex.EncodeToString(kvHash.Sum(nil)))
}
