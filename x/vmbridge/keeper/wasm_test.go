...
func (suite *KeeperTestSuite) TestKeeper_CallToWasm() {
	...
	testCases := []struct {
		...
		{
			"caller(20),wasmContract(ex 32),value(0),calldata(normal)",
			func() {
			},
			func() {
				queryAddr := caller
		...
		},
		{
			...
			func() {
				wasmContractAddr = hex.EncodeToString(suite.freeCallWasmContract.Bytes())
			},
			func() {
				queryAddr := caller
				result, err := suite.app.WasmKeeper.QuerySmart(suite.ctx, suite.freeCallWasmContract, []byte(fmt.Sprintf("{\"balance\":{\"address\":\"%s\"}}", queryAddr.String())))
		...
		},
		{
			...
			func() {
				wasmContractAddr = sdk.AccAddress(common.BytesToAddress(suite.freeCallWasmContract.Bytes()).Bytes()).String()
			},
			func() {
			},
			types.ErrIsNotWasmAddr,
		},
		{
			...
			func() {
				wasmContractAddr = common.BytesToAddress(suite.freeCallWasmContract.Bytes()).String()
			},
			func() {
			},
			types.ErrIsNotWasmAddr,
		...
		{
			"caller(20),wasmContract(0x 32),value(0),calldata(empty)",
			func() {
				calldata = ""
			},
			func() {
		...
		{
			"caller(20),wasmContract(0x 32),value(0),calldata(not standard schema)",
			func() {
		...
		{
			"caller(20),wasmContract(0x 32),value(0),calldata(not method schema)",
			func() {
		...
		{
			"caller(20),wasmContract(ex 32),value(1),calldata(normal)",
			func() {
		...
		{
			"caller(20),wasmContract(ex 32),value(2 insufficient),calldata(normal)",
			func() {
		...
		{
			"caller(20),wasmContract(ex 32),value(-1 negative),calldata(normal)",
			func() {
				value = sdk.NewIntFromBigInt(sdk.NewDec(-1).BigInt())
				suite.SetAccountCoins(caller, sdk.NewInt(1))
			},
...
func (suite *KeeperTestSuite) TestMsgServer_CallToEvmEvent() {
	...
	testCases := []struct {
		...
		{
			"caller(error),contract(0x 20),calldata(normal),amount(0)",
			func() {
		...
		{
			"caller(ex 32),contract(ex 20),calldata(normal),amount(0)",
			func() {
				contract = contractEx
			},
			func() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			msg := types.MsgCallToEvm{Sender: caller, Evmaddr: contract, Calldata: callData, Value: value}
			result, err := msgServer.CallToEvmEvent(sdk.WrapSDKContext(suite.ctx), &msg)
			if tc.error != nil {
				suite.Require().EqualError(err, tc.error.Error())
			} else {
				suite.Require().NoError(err)
				suite.Require().Equal(tc.expect, result.Response)
				tc.postcheck()
			}
		})
	}
