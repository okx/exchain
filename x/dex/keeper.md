...
| "tokenPairNumberKey"     |        uint64            |      1        |  dex运营方在okchain发行的币对数量  |  <1k |                | 存的okchain交易币对的数量 |
|     0x02+$owner_addr     |        WithdrawInfo      |  在途赎回数   |  dex运营方在okchain发行的币对数量  |  <1k |                | 存的okchain交易币对的数量 |
|     0x03+$trading_pair   |        product lock      |   币对数      |  dex运营方在okchain发行的币对数量  |  <1k |                | 存的okchain交易币对的数量 |
