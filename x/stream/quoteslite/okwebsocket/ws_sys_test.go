...
func TestLoadResponse(t *testing.T) {
	r, err := loadResponse([]byte("pong"))
	require.Nil(t, err, r, err)
}

func TestSubscriptionTopic(t *testing.T) {
	sr := FormSubscriptionTopic("dex_spot/all_ticker_3s")
	require.True(t, sr != nil)
	require.True(t, sr.Channel == "dex_spot/all_ticker_3s")
	require.True(t, sr.Filter == "")

	sr2 := FormSubscriptionTopic("dex_spot/all_ticker_3s:aaaaa")
	require.True(t, sr2 != nil)
	require.True(t, sr2.Channel == "dex_spot/all_ticker_3s")
	require.True(t, sr2.Filter == "aaaaa")
}
