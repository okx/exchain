...
func loadResponse(rspMsg []byte) (interface{}, error) {
	...
	if strings.ToLower(string(rspMsg)) == "pong" {
...
func printReceivedData(obj interface{}) error {
	switch obj.(type) {
	case string:
		fmt.Println("PrintReceivedData", obj)
	default:
		msg, err := Struct2JsonString(obj)
		if err != nil {
			fmt.Println("PrintReceivedData", err.Error(), obj)
			return err
		}
		fmt.Println("PrintReceivedData", msg)
