...
func (suite *UpgradeInfoStoreSuite) TestHandleUpgradeProposal() {
	tests := []struct {
		...
		{ // expect height is not zero but less than current height
			expectHeight: 10, currentHeight: 10, claimReady: false, expect1stExecuteError: true, expectHitError: true,
		},
		{ // expect height is not zero but only greater than current height 1; and not claim ready
			expectHeight: 21, currentHeight: 20, claimReady: false, expect1stExecuteError: true, expectHitError: true,
		},
		{ // expect height is not zero and greater than current height; but not claim ready
			expectHeight: 32, currentHeight: 30, claimReady: false, expect1stExecuteError: false, expectHitError: true,
		},
		{ // everything's ok: expect height is not zero and greater than current height; and claim ready
			expectHeight: 42, currentHeight: 40, claimReady: true, expect1stExecuteError: false, expectHitError: false,
		},
		{ // everything's ok: expect height is zero and claim ready
			expectHeight: 0, currentHeight: 50, claimReady: true, expect1stExecuteError: false, expectHitError: false,
		},
		{ // everything's ok: expect height is not zero and equal to current height; and claim ready
			expectHeight: 60, currentHeight: 60, claimReady: true, expect1stExecuteError: false, expectHitError: false,
		},
		{ // everything's ok: expect height is not zero and less than current height; and claim ready
			expectHeight: 70, currentHeight: 72, claimReady: true, expect1stExecuteError: false, expectHitError: false,
		},
	}
