...
func (k Keeper) initializeDelegation(ctx sdk.Context, vals []sdk.ValAddress, del sdk.AccAddress) {
	for _, val := range vals {
		...
		//validator := k.stakingKeeper.Validator(ctx, val) //TODO zhujianguo
		//delegation := k.stakingKeeper.Delegator(ctx, del) //TODO zhujianguo

		// calculate delegation stake in tokens
		// we don't store directly, so multiply delegation shares * (tokens per share)
		// note: necessary to truncate so we don't allow withdrawing more rewards than owed
		stake := sdk.Dec{} //validator.TokensFromSharesTruncated(delegation.GetShares()) //TODO zhujianguo
...
func (k Keeper) calculateDelegationRewards(ctx sdk.Context, val stakingexported.ValidatorI, del stakingexported.DelegatorI, endingPeriod uint64) (rewards sdk.DecCoins) {
	//TODO zhujianguo for each
	for _, varAddr := range del.GetShareAddedValidatorAddresses() {
		// fetch starting info for delegation
		startingInfo := k.GetDelegatorStartingInfo(ctx, varAddr, del.GetDelegatorAddress())
		...
		if endingHeight > startingHeight {
			k.IterateValidatorSlashEventsBetween(ctx, varAddr, startingHeight, endingHeight,
		...
		currentStake := val.TokensFromShares(del.GetLastAddedShares())
...
func (k Keeper) withdrawDelegationRewards(ctx sdk.Context, val stakingexported.ValidatorI, del stakingexported.DelegatorI) (sdk.Coins, error) {
	// check existence of delegator starting info
	if !k.HasDelegatorStartingInfo(ctx, val.GetOperator(), del.GetDelegatorAddress()) {
	...
	if !rewards.IsEqual(rewardsRaw) {
		...
		logger.Info(fmt.Sprintf("missing rewards rounding error, delegator %v"+
			...
			val.GetOperator(), del.GetDelegatorAddress(), rewardsRaw, rewards))
	}
	...
	if !coins.IsZero() {
		withdrawAddr := k.GetDelegatorWithdrawAddr(ctx, del.GetDelegatorAddress())
	...
	k.DeleteDelegatorStartingInfo(ctx, val.GetOperator(), del.GetDelegatorAddress())
