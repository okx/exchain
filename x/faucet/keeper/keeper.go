...
func (k Keeper) GetCap(ctx sdk.Context) sdk.Dec {
	store := ctx.KVStore(k.storeKey)
...
func (k Keeper) SetCap(ctx sdk.Context, cap sdk.Dec) {
	store := ctx.KVStore(k.storeKey)
	bz := k.cdc.MustMarshalBinaryBare(cap)
	store.Set(types.CapKey, bz)
}

func (k Keeper) GetMaxPerRequest(ctx sdk.Context) sdk.Dec {
	store := ctx.KVStore(k.storeKey)
	...
	var maxPerReq sdk.Dec
	k.cdc.MustUnmarshalBinaryBare(bz, &maxPerReq)

	return maxPerReq
}

func (k Keeper) SetMaxPerRequest(ctx sdk.Context, maxPerReq sdk.Dec) {
	store := ctx.KVStore(k.storeKey)
