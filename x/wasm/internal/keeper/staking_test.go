...
func initializeStaking(t *testing.T) initInfo {
	...
	valAddr := addValidator(ctx, stakingKeeper, accKeeper, sdk.NewDecCoinFromDec("stake", sdk.MustNewDecFromStr("100000000")))
	ctx = nextBlock(ctx, stakingKeeper)
	...
	v, found := stakingKeeper.GetValidator(ctx, valAddr)
	assert.True(t, found)
	assert.Equal(t, v.GetDelegatorShares(), sdk.NewDec(100000000))
	assert.Equal(t, v.Status, sdk.Bonded)

	//deposit := sdk.NewCoins(sdk.NewInt64Coin("denom", 100000), sdk.NewInt64Coin("stake", 500000))
	deposit := sdk.NewCoins(sdk.NewDecCoinFromDec("denom", sdk.MustNewDecFromStr("100000")), sdk.NewDecCoinFromDec("stake", sdk.MustNewDecFromStr("500000")))
	creator := createFakeFundedAccount(ctx, accKeeper, deposit)
...
func TestReinvest(t *testing.T) {
	...
	require.True(t, found)
	initPower := val.GetDelegatorShares()
	assert.Equal(t, val.Tokens, sdk.NewInt(100000000), "%s", val.Tokens)
...
func addValidator(ctx sdk.Context, stakingKeeper staking.Keeper, accountKeeper auth.AccountKeeper, value sdk.Coin) sdk.ValAddress {
	...
	owner := createFakeFundedAccount(ctx, accountKeeper, sdk.NewDecCoinsFromDec(value.Denom, value.Amount))
