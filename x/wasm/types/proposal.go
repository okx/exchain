...
func (p StoreCodeProposal) ValidateBasic() error {
	...
	if _, err := AccAddressFromBech32(p.RunAs); err != nil {
...
func (p InstantiateContractProposal) ValidateBasic() error {
	...
	if len(p.Admin) != 0 {
		if _, err := AccAddressFromBech32(p.Admin); err != nil {
			return err
		}
	}
...
func (p MigrateContractProposal) ValidateBasic() error {
	...
	if _, err := AccAddressFromBech32(p.Contract); err != nil {
...
func (p UpdateAdminProposal) ValidateBasic() error {
	...
	if _, err := AccAddressFromBech32(p.NewAdmin); err != nil {
