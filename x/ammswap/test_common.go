...
func NewTestSwapTokenPairWithInitLiquidity(t *testing.T, ctx sdk.Context, k swapkeeper.Keeper,
	baseToken, quoteToken sdk.DecCoin, addr sdk.AccAddress) SwapTokenPair {
	handler := NewHandler(k)

	createExchangeMsg := types.NewMsgCreateExchange(baseToken.Denom, quoteToken.Denom, addr)
	result := handler(ctx, createExchangeMsg)
	require.Equal(t, true, result.IsOK())

	deadLine := time.Now().Unix()
	addLiquidityMsg := types.NewMsgAddLiquidity(sdk.NewDec(0), baseToken, quoteToken, deadLine, addr)
